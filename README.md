...
The website of De Lübsche Schut (Lachswehrallee 40, Lübeck) — a Next.js 16 site
whose content is edited in a Payload CMS admin panel at `/admin`.
...
| --- | --- |
| **Startseite** | Every section of the homepage — Hero, Über uns, Wer wir sind, Galerie, Kontakt |
| **Einstellungen** | Address, phone, e-mail, opening hours, SEO metadata, the menu PDF link, footer marquee |
...
The owner describes a change in German ("Übernimm die neuen Öffnungszeiten von
diesem Foto"), optionally attaches up to 5 photos/PDFs, and saves. The request
goes to Claude together with the current content and the attachments (images as
vision, PDFs as documents); the returned content is written back through Payload,
which re-validates every field. Needs `ANTHROPIC_API_KEY`.
...
`src/lib/defaults.ts` holds the site's content as it was before the CMS. The seed
writes exactly that into the globals, and `src/lib/content.ts` falls back to it
field by field — so an unseeded database still renders the original site rather
than an empty one.
