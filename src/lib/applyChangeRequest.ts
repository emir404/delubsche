...
// the site's content: the current Homepage and SiteSettings globals go to Claude
// as JSON, the attachments go along as readable content (images as vision, PDFs
// as documents), and the returned content is written back through Payload —
// which re-validates every field against the schema.
...
const SYSTEM = [
  "You are the content editor for the website of De Lübsche Schut, a restaurant on a barge on the",
  "Trave in Lübeck (German and Mexican cuisine).",
  "You receive the site's entire editable content as two JSON values — `homepage` and `siteSettings`",
  "— plus the owner's request in German, often with attached photos or PDFs (a new menu, new opening",
  "hours, a flyer). Read the attachments carefully and apply exactly the requested changes to the JSON.",
  "",
  "Rules:",
  "- All visible text is German — keep it German, in the site's warm, personal voice (Ihr/Euch-Ansprache).",
  "- Change ONLY what the request implies. Copy every other value through unchanged, byte for byte.",
  "- When an attachment contains content (opening hours, names, prices), transcribe it faithfully —",
  ...
  "  en-dash „–“ with spaces (never a hyphen), the same label patterns, the same capitalization,",
  "  punctuation, and roughly the same text length as what was there before. Headings are UPPERCASE",
  "  in the design; keep the case of the value you replace.",
  "- Numeric fields that reference images (e.g. `image`, `imageLarge`, `gallery.images[].image`,",
  "  `owners.people[].image`) are upload IDs. NEVER change, add, or remove them, and never turn",
  "  them into strings.",
  ...
  '  with `time` (empty both = closed that day, "24:00" = open end).',
  "- Fields ending in `heading` may contain line breaks — each line is animated separately, so keep",
  "  the existing number of lines unless the request needs otherwise.",
  "- Respect the array limits: hero.columns ≤ 3, owners.people = exactly 2, gallery.images ≤ 12,",
  "  contact.infoItems ≤ 4.",
  "- If the request cannot be fulfilled with these fields (e.g. it needs a new page or new images),",
  '  set "homepage" and "siteSettings" to null and explain why in "summary" (German).',
  "",
...
export async function applyChangeRequest(input: {
  ...
  if (failures.length && !changedHomepage && !changedSettings) {
